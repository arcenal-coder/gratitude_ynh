...
async function renderAdmin() {
  ...
  document.querySelector("[data-moderation-toggle]")?.addEventListener("change", updateModeration);
}
...
async function updateModeration(event) { try { const settings = await api("/settings", { method: "POST", body: JSON.stringify({ require_moderation: event.currentTarget.checked }) }); toast(settings.require_moderation ? "Validation préalable activée." : "Publication immédiate activée."); } catch (error) { event.currentTarget.checked = !event.currentTarget.checked; toast(error.message, true); } }
